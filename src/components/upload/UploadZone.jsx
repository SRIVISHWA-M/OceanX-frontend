    ...
    <div className="w-full h-full flex flex-col items-center justify-center gap-6 py-4">
      ...
      <div
        className={`relative w-full max-w-xl rounded-3xl border-2 border-dashed transition-all duration-500 cursor-pointer group ${
          dragActive
            ? 'border-blue-400 bg-blue-50/80 scale-[1.01] shadow-xl shadow-blue-100'
            : 'border-blue-100 bg-white hover:border-blue-300 hover:bg-blue-50/30 hover:shadow-lg hover:shadow-blue-50'
        }`}
        onDragEnter={onDrag}
        onDragLeave={onDrag}
        onDragOver={onDrag}
      ...
      >
        ...
        <div className="absolute -top-6 -right-6 w-32 h-32 bg-blue-100/60 rounded-full blur-2xl pointer-events-none transition-opacity group-hover:opacity-80" />
        ...
        <div className="relative flex flex-col items-center text-center px-8 py-10">
          ...
          <div className="relative mb-6">
            {/* Ripple rings */}
            <div className={`absolute inset-0 rounded-3xl bg-blue-300/20 ${dragActive ? 'animate-ripple' : 'group-hover:animate-ripple'}`} />
            <div className={`absolute inset-0 rounded-3xl bg-blue-200/15 ${dragActive ? 'animate-ripple' : 'group-hover:animate-ripple'}`} style={{ animationDelay: '0.4s' }} />

            {/* Main icon */}
            <div className={`relative h-20 w-20 rounded-3xl bg-gradient-to-br from-blue-600 to-blue-400 flex items-center justify-center text-4xl shadow-2xl shadow-blue-200/60 transition-transform duration-500 ${dragActive ? 'scale-110 rotate-6' : 'group-hover:scale-105 group-hover:rotate-3'}`}>
              {dragActive ? '🎓' : '📚'}
            </div>
          </div>
          ...
          <button
            ...
            className="relative px-7 py-3 rounded-2xl bg-gradient-to-r from-blue-600 to-blue-500 text-white font-bold text-sm shadow-lg shadow-blue-200/60 hover:shadow-blue-300/70 hover:scale-[1.03] active:scale-95 transition-all overflow-hidden group/btn"
          >
            {/* Shimmer sweep */}
      ...
      <div className="flex flex-wrap justify-center gap-2">
        {[
          ...
          { icon: '💬', label: 'Text Notes', color: 'bg-sky-50 text-sky-600 border-sky-100' },
